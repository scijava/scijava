...
public class LazilyGeneratedMethodParameterData implements ParameterData {
		...
		List<FunctionalMethodType> fmts, MethodJavadoc doc, Method m)
	{
		...
		List<FunctionalMethodType> returns = fmts.stream().filter(fmt -> fmt.itemIO() == ItemIO.OUTPUT).collect(Collectors.toList());
		if (returns.size() > 1) throw new IllegalStateException("A method can only have one return but this one has multiple!");
		if (returns.size() == 1) {
