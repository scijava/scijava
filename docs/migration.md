...
| `io.nio` | **not ported.** `NIOService`/`DefaultNIOService` wrap two JDK calls (`ByteBuffer.allocate` + `FileChannel.read`, or `FileChannel.map`) behind a system-property switch, and have no usages in the scijava or imagej repositories. `ByteBufferByteBank` also has none, and caps at `Integer.MAX_VALUE` like `ByteArrayByteBank`, so it adds no capability — only a pluggable allocator, e.g. `ByteBuffer::allocateDirect`. `ByteBank` is ported, so a direct-buffer bank is a small class to add when something needs one. NB: SCIFIO was not checked, having no local checkout | dropped |
