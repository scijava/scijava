	...
	private static void parseFieldParameters (final ArrayList<Member<?>> items, final Set<String> names, final ArrayList<ValidityProblem> problems,
		...
		final List<Field> fields = ClassUtils.getAnnotatedFields(annotatedClass,
			...
			for (final Field f : fields) {
				...
				// for field parameters (aka secondary parameters), the ItemIO can't be inferred automatically
				if (param.itemIO().equals(ItemIO.AUTO)) {
					problems.add(new ValidityProblem("The ItemIO of the annotated field: '" + f.toString()
							+ "' must be specified. It must not be AUTO."));
				}
				...
				try {
					final ParameterMember<?> item = new FieldParameterMember<>(f, annotatedClass, param);
					names.add(name);
					items.add(item);
				}
