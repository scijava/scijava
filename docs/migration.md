...
| `BasicDetails`, `AbstractBasicDetails`, `UIDetails`, `AbstractUIDetails`, `MenuEntry`, `MenuPath` | `scijava-command3` presentation metadata, as attributes in the annotation index rather than an inheritance chain every plugin extends. **Done**: menu path, label, icon, accelerator, weight, visibility — carried by `@Menu`, exposed on `CommandInfo`, rendered by all three bindings. **Declared but not yet rendered**: `selectable` and `selectionGroup` reach the index and stop there. **Not migrated, by design**: `isSelected`/`isEnabled`, which in SciJava Common are *mutable setters on the description* — the same mutability that made `ModuleInfo` per-instance. They are application state rather than metadata; see the presentation-state section of [migration-plan.md](migration-plan.md). `menuRoot` awaits an application with more than one menu root | open |
...
| `ui` | `scijava-ui3` holds the toolkit-agnostic contracts, and the widget half is done. `ApplicationFrame`/`Desktop`/`StatusBar`/`ToolBar` are still to come, as contracts there with implementations in the toolkit bindings. Dropped from the *core*, not from the stack — see the layering section of [migration-plan.md](migration-plan.md) | planned |
